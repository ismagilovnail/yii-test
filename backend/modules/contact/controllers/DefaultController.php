...
class DefaultController extends Controller
{
    ...
    public function actionModerate($id)
    {
        if (Yii::$app->request->post()) {
            ...
            $model->status = true;
            if ($model->update()) {
                Yii::$app->session->setFlash('success', 'Заявка ID'.$model->id.' успешно одобрена пользователь оповещен по email: '. $model->user->email.'!');
            }else{
                Yii::$app->session->setFlash('danger', 'Ошибка модерации ID '.$model->id);
            }
        }
